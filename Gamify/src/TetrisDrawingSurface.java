	...
	public void keyPressed() { 
		...
		if (!paused) {
			if (key == ' ') {
				...
				while (translatable) {
					for (int i = 0; i < 4; i++) {
						if (board[activePiece.getY(i) + activePiece.getYShift()+1][activePiece.getX(i)+activePiece.getXShift()]>0) {
							translatable = false;
						}
					}
			...
			if (key == CODED) {
				...
				if (keyCode == LEFT) {
					for (int i = 0; i < 4; i++) {
						if (activePiece.getX(i)+activePiece.getXShift()-1 < 0 || board[activePiece.getY(i)+activePiece.getYShift()][activePiece.getX(i)+activePiece.getXShift()-1] > 0) {
							translatable = false;
						}
					}
					if (translatable) {
					...
					for (int i = 0; i < 4; i++) {
						if (activePiece.getX(i)+activePiece.getXShift()+1>11 ||  board[activePiece.getY(i)+activePiece.getYShift()][activePiece.getX(i)+activePiece.getXShift()+1] > 0) {
							translatable = false;
						}
					}
					if (translatable) {
